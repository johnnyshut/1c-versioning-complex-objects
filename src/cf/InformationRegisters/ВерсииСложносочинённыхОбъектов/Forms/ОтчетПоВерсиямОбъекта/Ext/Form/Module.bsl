...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе // Используется переданная версия объекта.
		...
		Если Параметры.ПоВерсии Тогда // Используется отчет по одной версии.
			ТаблицаОтчета = ВерсионированиеСложносочинённыхОбъектов.ОтчетПоВерсииОбъекта(СсылкаНаОбъект, СериализованныйОбъект);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если СравниваемыеВерсии.Количество() > 1 Тогда
		...
		Для Каждого Версия Из СравниваемыеВерсии Цикл
			...
			ВерсияОбъекта = ВерсионированиеСложносочинённыхОбъектов.СведенияОВерсииОбъекта(СсылкаНаОбъект, Версия.Значение).ВерсияОбъекта;
...
Функция СформироватьОтчетПоВерсиям()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("РегистрыСведений.ВерсииСложносочинённыхОбъектов.СформироватьОтчетПоИзменениям", ПараметрыОтчета, ПараметрыВыполнения);
КонецФункции
...
Процедура СформироватьОтчетПоВерсии()
	ТаблицаОтчета = ВерсионированиеСложносочинённыхОбъектов.ОтчетПоВерсииОбъекта(СсылкаНаОбъект, СравниваемыеВерсии[0].Значение, СравниваемыеВерсии[0].Представление);
КонецПроцедуры
